...
<div class="content-header fade-in">
    <h1 class="fw-bold text-primary">Edit Item</h1>
    <p class="text-muted">Update the item details.</p>
</div>
                    ...
                    <div class="mb-4 position-relative">
                        ...
                        <select name="service_category_id" id="service_category_id" class="form-control form-control-lg rounded-3 @error('service_category_id') is-invalid @enderror" required>
                            ...
                            @foreach ($serviceCategories as $serviceCategory)
                                <option value="{{ $serviceCategory->id }}" {{ old('service_category_id', $yourItem->service_category_id) == $serviceCategory->id ? 'selected' : '' }}>{{ $serviceCategory->name['en'] }}</option>
                            @endforeach
                        </select>
