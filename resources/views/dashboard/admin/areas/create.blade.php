...
<div class="content-header fade-in">
    <h1 class="fw-bold text-primary" data-en="Add New Area" data-ar="إضافة منطقة جديدة">Add New Area</h1>
    <p class="text-muted" data-en="Create a new area with multilingual names and an optional description." data-ar="إنشاء منطقة جديدة بأسماء متعددة اللغات ووصف اختياري.">Create a new area with multilingual names and an optional description.</p>
</div>
...
<div class="card shadow-lg border-0" style="background: linear-gradient(145deg, #ffffff, #f8fafc);">
    <div class="card-header bg-primary text-white py-3 rounded-top">
        <h5 class="card-title mb-0" data-en="Create Area" data-ar="إنشاء منطقة">Create Area</h5>
    </div>
    <div class="card-body p-4">
        <form action="{{ route('admin.areas.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" id="slug" class="form-control form-control-lg rounded-3 @error('slug') is-invalid @enderror" value="{{ old('slug') }}">
                @error('slug')
                    <div class="invalid-feedback d-flex align-items-center"><i class="fas fa-exclamation-circle me-2"></i>{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" class="form-control form-control-lg rounded-3 @error('description') is-invalid @enderror" rows="4">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback d-flex align-items-center"><i class="fas fa-exclamation-circle me-2"></i>{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" min="0" name="price_increase_percentage" id="price_increase_percentage" class="form-control form-control-lg rounded-3 @error('price_increase_percentage') is-invalid @enderror" value="{{ old('price_increase_percentage', 0) }}" placeholder="e.g., 10.00">
            ...
            <div class="d-flex gap-3 mt-4">
                <button type="submit" class="btn btn-primary btn-lg" data-en="Save Area" data-ar="حفظ المنطقة">
