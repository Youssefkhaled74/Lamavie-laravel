...
<div class="container-fluid">
    ...
    <div class="content-header fade-in mb-4 d-flex align-items-center justify-content-between">
        <div>
            <h1 class="fw-bold text-primary mb-1" data-en="Edit Service Type" data-ar="تعديل نوع الخدمة">Edit Service Type</h1>
            <p class="text-muted mb-0" data-en="Update the service type details." data-ar="تحديث بيانات نوع الخدمة.">Update the service type details.</p>
        </div>
    ...
    <div class="card shadow-lg border-0 rounded-4 overflow-hidden" style="background: linear-gradient(145deg, #ffffff, #f8fafc);">
        <div class="card-header py-3 px-3 text-white d-flex justify-content-between align-items-center"
            ...
            <h5 class="card-title mb-0" data-en="Edit Service Type" data-ar="تعديل نوع الخدمة">Edit Service Type</h5>
            <a href="{{ route('admin.service-types.show', $serviceType) }}" class="btn btn-light btn-sm">
        ...
        <div class="card-body p-4" style="max-height: calc(100vh - 220px); overflow-y: auto;">
            <form action="{{ route('admin.service-types.update', $serviceType) }}" method="POST" enctype="multipart/form-data" id="service-type-form">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text"
                               ...
                               value="{{ old('name_en', data_get($serviceType,'name.en','')) }}"
                               required>
                        @error('name_en')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text"
                               ...
                               value="{{ old('name_ar', data_get($serviceType,'name.ar','')) }}"
                               required>
                        @error('name_ar')
                            <div class="invalid-feedback d-flex align-items-center"><i class="fas fa-exclamation-circle me-2"></i>{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="service_id"
                                ...
                                required>
                            <option value="" disabled data-en="Select a service" data-ar="اختر خدمة">Select a service</option>
                            @foreach ($services as $service)
                                @php
                                    $enName = data_get($service, 'name.en', '');
                                    $arName = data_get($service, 'name.ar', '');
                                @endphp
                                <option value="{{ $service->id }}" {{ old('service_id', $serviceType->service_id) == $service->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($serviceType->logo)
                            <div class="mb-2 d-flex align-items-center gap-2">
                                <img src="{{ Storage::url($serviceType->logo) }}" class="rounded-3 border shadow-sm" style="width:64px;height:64px;object-fit:cover;">
                                <div class="small text-muted" data-en="Current logo" data-ar="الشعار الحالي">Current logo</div>
                            </div>
                        @endif
                ...
                <div class="d-flex gap-3 mt-4">
                    <button type="submit" class="btn btn-primary btn-lg" id="submit-btn">
                        ...
                        <span data-en="Update Service Type" data-ar="تحديث نوع الخدمة">Update Service Type</span>
                    </button>
