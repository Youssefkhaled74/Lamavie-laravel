...
<div class="content-header fade-in">
    <h1 class="fw-bold text-primary">Measurements</h1>
    <p class="text-muted">Manage measurements with their associated service categories.</p>
</div>
...
<div class="card shadow-lg border-0" style="background: linear-gradient(145deg, #ffffff, #f8fafc);">
    <div class="card-header bg-primary text-white py-3 rounded-top d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Measurements List</h5>
        <a href="{{ route('admin.measurement.create') }}" class="btn btn-light btn-sm">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered">
                ...
                <tbody id="items-table-body">
                    @include('dashboard.admin.measurement.partials.items-table', ['measurements' => $measurements])
                </tbody>
            </table>
        </div>
        <div id="pagination-links">
            {{ $measurements->appends(['service_category_id' => request()->service_category_id])->links('vendor.pagination.bootstrap-5') }}
        </div>
    </div>
</div>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function loadItems(serviceCategoryId = '', page = 1) {
            let url = '{{ route('admin.measurement.index') }}';
            ...
            .catch(error => {
                ...
                tableBody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Error loading measurements.</td></tr>';
            });
        }
