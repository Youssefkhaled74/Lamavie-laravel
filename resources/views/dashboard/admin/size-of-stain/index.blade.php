...
<div class="content-header fade-in">
    <h1 class="fw-bold text-primary">Size of Stains</h1>
    <p class="text-muted">Manage size of stains with their associated service categories.</p>
</div>
...
<div class="card shadow-lg border-0" style="background: linear-gradient(145deg, #ffffff, #f8fafc);">
    <div class="card-header bg-primary text-white py-3 rounded-top d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Size of Stains List</h5>
        <a href="{{ route('admin.size-of-stain.create') }}" class="btn btn-light btn-sm">
            <i class="fas fa-plus me-2"></i>Add New Size of Stain
        </a>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered">
                ...
                <tbody id="items-table-body">
                    @include('dashboard.admin.size-of-stain.partials.items-table', ['sizeOfStains' => $sizeOfStains])
                </tbody>
            </table>
        </div>
        <div id="pagination-links">
            {{ $sizeOfStains->appends(['service_category_id' => request()->service_category_id])->links('vendor.pagination.bootstrap-5') }}
        </div>
    </div>
</div>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function loadItems(serviceCategoryId = '', page = 1) {
            let url = '{{ route('admin.size-of-stain.index') }}';
            ...
            .catch(error => {
                console.error('Error loading size of stains:', error);
                tableBody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Error loading size of stains.</td></tr>';
            });
        }
