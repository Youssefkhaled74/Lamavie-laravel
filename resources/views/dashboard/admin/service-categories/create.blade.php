...
<div class="content-header fade-in">
    <h1 class="fw-bold text-primary">Add New Service Category</h1>
    <p class="text-muted">Create a new service category with multilingual names and logo.</p>
</div>
...
<div class="card shadow-lg border-0" style="background: linear-gradient(145deg, #ffffff, #f8fafc);">
    ...
    <div class="card-body p-4" style="max-height: calc(100vh - 220px); overflow-y: auto;">
        <form action="{{ route('admin.service-categories.store') }}" method="POST" enctype="multipart/form-data" id="service-category-form">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="mb-4 position-relative">
                        ...
                        <select name="service_id" id="service_id" class="form-control form-control-lg rounded-3 @error('service_id') is-invalid @enderror" required>
                            ...
                            @foreach ($services as $service)
                                <option value="{{ $service->id }}" {{ old('service_id') == $service->id ? 'selected' : '' }}>{{ $service->name['en'] }}</option>
                            @endforeach
                        </select>
                        @error('service_id')
                            <div class="invalid-feedback d-flex align-items-center"><i class="fas fa-exclamation-circle me-2"></i>{{ $message }}</div>
                ...
                <div class="col-md-6">
                    <div class="mb-4">
                        ...
                        @error('logo')
                            <div class="invalid-feedback d-flex align-items-center"><i class="fas fa-exclamation-circle me-2"></i>{{ $message }}</div>
                        @enderror
                        <div id="logo-preview" class="mt-3 d-flex flex-wrap gap-3" style="max-height: 200px; overflow-y: auto;"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex gap-3 mt-4">
                <button type="submit" class="btn btn-primary btn-lg" id="submit-btn">
                    <i class="fas fa-save me-2"></i>Save Service Category
                </button>
